    ...
    vector<int>& legal, const STATUS& status) const
{
    ...
    else if( Knowledge.Level(status.Phase)== 4)
    {
    	for (int a = 0; a < NumActions; ++a)
            if (!bsstate.Cells(a).Visited && !( bsstate.Cells(a-10).Visited && bsstate.Cells(a+10).Visited && bsstate.Cells(a-1).Visited && bsstate.Cells(a+1).Visited) && !bsstate.Cells(a).Diagonal)
                legal.push_back(a);
    }
    else if( Knowledge.Level(status.Phase)== 5)
    {
    	for (int a = 0; a < NumActions; ++a)
            if (!bsstate.Cells(a).Visited && !( bsstate.Cells(a-10).Visited && bsstate.Cells(a+10).Visited && bsstate.Cells(a-1).Visited && bsstate.Cells(a+1).Visited) )
                legal.push_back(a);
    }
    else if( Knowledge.Level(status.Phase)== 6)
    {
    	for (int a = 0; a < NumActions; ++a)
    	{
    		int x= a % 10 ;
            ...
            if (!bsstate.Cells(a).Visited 
            	&& ( x-1<0 || ( x-1>=0 && (!bsstate.Cells( y*10 + x ).Visited) ) )
            	&& ( y-1<0 || ( y-1>=0 && (!bsstate.Cells( (y-1)*10 + x +1).Visited) ) )
            	&& ( x+1>=10 || ( x+1<10 && (!bsstate.Cells( y*10 + x+2 ).Visited) ) )
            	&& ( y+1>=10 || ( y+1<10 && (!bsstate.Cells( (y+1)*10 + x +1).Visited) ) )
            	)
                legal.push_back(a);
    	}
    }
